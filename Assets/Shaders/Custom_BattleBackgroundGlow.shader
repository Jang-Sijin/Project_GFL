...
//
///////////////////////////////////////////
Shader "Custom/BattleBackgroundGlow" {
Properties {
...
SubShader {
 Tags { "CanUseSpriteAtlas" = "true" "IGNOREPROJECTOR" = "true" "LIGHTMODE" = "FORWARDBASE" "QUEUE" = "Transparent" "RenderEffect" = "Glow11Transparent" "RenderType" = "Glow11Transparent" }
 Pass {
  Tags { "CanUseSpriteAtlas" = "true" "IGNOREPROJECTOR" = "true" "LIGHTMODE" = "FORWARDBASE" "QUEUE" = "Transparent" "RenderEffect" = "Glow11Transparent" "RenderType" = "Glow11Transparent" }
...
void main()
{
    ...
    u_xlat3.x = float(1.0) / _FarAddDistance;
    u_xlat0.x = u_xlat3.x * u_xlat0.x;
    u_xlat0.x = clamp(u_xlat0.x, 0.0, 1.0);
    u_xlat3.x = u_xlat0.x * -2.0 + 3.0;
    u_xlat0.x = u_xlat0.x * u_xlat0.x;
    u_xlat0.x = u_xlat0.x * u_xlat3.x;
    u_xlat1.xy = _Time.xx * vec2(_MoveSpeedY, _MoveSpeed) + vs_TEXCOORD0.yx;
    u_xlatb3.x = u_xlat1.x>=(-u_xlat1.x);
    u_xlat6 = fract(abs(u_xlat1.x));
    u_xlat3.x = (u_xlatb3.x) ? u_xlat6 : (-u_xlat6);
    u_xlat6 = (-_Ydown) + _Yup;
    u_xlat1.z = abs(u_xlat3.x) * u_xlat6 + _Ydown;
    u_xlat10_1 = texture2D(_MainTex, u_xlat1.yz);
    u_xlat1 = u_xlat10_1 * vec4(vec4(_Bright, _Bright, _Bright, _Bright));
    u_xlat1 = u_xlat1 * _Color;
    u_xlat1 = u_xlat1 * vs_COLOR0;
    u_xlat3.x = dot(u_xlat1.xyz, vec3(0.298999995, 0.587000012, 0.114));
    u_xlat2.y = _Time.x * 100.0 + (-u_xlat3.x);
    u_xlat2.x = 0.0;
    u_xlat3.xy = u_xlat2.xy + vs_TEXCOORD2.xy;
    u_xlat10_3 = texture2D(_ScreenTex, u_xlat3.xy).w;
    u_xlat2 = vec4(u_xlat10_3) * _SColor;
    u_xlat3.x = _Time.x * 50.0;
    u_xlatb3.x = u_xlat3.x>=vs_TEXCOORD0.x;
    u_xlat6 = _Time.x * 50.0 + -0.200000003;
    u_xlatb3.y = vs_TEXCOORD0.x>=u_xlat6;
    u_xlat3.xy = mix(vec2(0.0, 0.0), vec2(1.0, 1.0), vec2(u_xlatb3.xy));
    u_xlat3.x = u_xlat3.y * u_xlat3.x;
    u_xlat1 = u_xlat2 * u_xlat3.xxxx + u_xlat1;
    u_xlat10_3 = texture2D(_AlphaTex, vs_TEXCOORD1.xy).x;
    u_xlat3.x = u_xlat10_3 * u_xlat1.w;
    u_xlat3.x = u_xlat3.x * _Color.w;
    u_xlat2.w = u_xlat3.x * vs_COLOR0.w;
    u_xlat3.xyz = u_xlat1.xyz * u_xlat2.www;
    u_xlat1.xyz = (-u_xlat1.xyz) * u_xlat2.www + _FarFogColor.xyz;
    u_xlat2.xyz = u_xlat0.xxx * u_xlat1.xyz + u_xlat3.xyz;
    SV_Target0 = u_xlat2;
    return;
}
