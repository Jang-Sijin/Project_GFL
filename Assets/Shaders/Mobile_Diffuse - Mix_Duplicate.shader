...
//
///////////////////////////////////////////
Shader "Mobile/Diffuse - Mix_Duplicate" {
Properties {
_MainTex ("Base (RGB)", 2D) = "white" { }
_SubTex ("CrossFade", 2D) = "white" { }
...
void main()
{
    ...
    u_xlat0.xy = in_TEXCOORD2.xy * _ScreenTex_ST.xy + _ScreenTex_ST.zw;
    u_xlat0.zw = in_TEXCOORD3.xy * _SubTex_ST.xy + _SubTex_ST.zw;
...
void main()
{
    ...
    u_xlat4.x = _Time.x * 50.0 + -0.200000003;
    u_xlatb0.y = vs_TEXCOORD0.x>=u_xlat4.x;
    u_xlat0.xy = mix(vec2(0.0, 0.0), vec2(1.0, 1.0), vec2(u_xlatb0.xy));
    u_xlat0.x = u_xlat0.y * u_xlat0.x;
    u_xlat4.x = vs_TEXCOORD0.x + (-_MainTex_ST.z);
    u_xlat4.x = u_xlat4.x / _MainTex_ST.x;
    u_xlatb4 = _FadePoint<u_xlat4.x;
    u_xlat1.x = _Time.x * _MoveSpeed + vs_TEXCOORD3.x;
    u_xlat8 = (-_Ydown) + _Yup;
    u_xlat1.y = vs_TEXCOORD3.y * u_xlat8 + _Ydown;
    u_xlat2.y = vs_TEXCOORD0.y * u_xlat8 + _Ydown;
    u_xlat1 = texture2D(_SubTex, u_xlat1.xy);
    u_xlat2.x = _Time.x * _MoveSpeed + vs_TEXCOORD0.x;
    u_xlat3 = texture2D(_MainTex, u_xlat2.xy);
    u_xlat10_8 = texture2D(_AlphaTex, u_xlat2.xy).w;
    u_xlat1 = (bool(u_xlatb4)) ? u_xlat1 : u_xlat3;
    u_xlat1 = u_xlat1 * vec4(vec4(_Bright, _Bright, _Bright, _Bright));
    u_xlat1 = u_xlat1 * _Color;
    u_xlat1 = u_xlat1 * vs_COLOR0;
    u_xlat4.x = dot(u_xlat1.xyz, vec3(0.298999995, 0.587000012, 0.114));
    u_xlat2.y = _Time.x * 100.0 + (-u_xlat4.x);
    u_xlat2.x = 0.0;
    u_xlat4.xz = u_xlat2.xy + vs_TEXCOORD2.xy;
    u_xlat10_4 = texture2D(_ScreenTex, u_xlat4.xz).w;
    u_xlat2 = vec4(u_xlat10_4) * _SColor;
    u_xlat1 = u_xlat2 * u_xlat0.xxxx + u_xlat1;
    u_xlat0 = vec4(u_xlat10_8) * u_xlat1;
